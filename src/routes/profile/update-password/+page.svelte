      ...
      {#if !invalidLink}
        ...
        <h3 class="text-2xl font-bold mb-3">Update password</h3>
          ...
          <form method="POST" >
            ...
            <button class="btn btn-neutral w-full shadow-none mt-8" disabled={!passwordValid || (password != confirmPassword)}>Update password</button>
          </form>

        {:else}
          <div class="flex flex-col">
            <h3 class="text-2xl font-bold mb-3 self-center">Link invalid or expired</h3>
            
            <a class="btn btn-neutral btn-wide shadow-none self-center mt-3" href="/login">Get other one</a>
          </div>
        {/if}
    </div>
  </div>
