<script lang="ts">
  ...
  const highlightSearch = (text) =>
    text.replaceAll(
      searchString,
      '<mark class="highlight">' + searchString + "</mark>",
    );

  const highlightHiddenSearch = (text) =>
    text.replaceAll(
      searchString,
      '<mark class="highlight-hidden">' + searchString + "</mark>",
    );
...
<VocabModal bind:showModal {vocab} {folders} {currentFolderId} />
        ...
        {#each items as item, i}
          ...
          <div class="card bg-base-100 card-border w-50 h-36 m-1 relative">
            {#if userState.select}
              <input
                type="checkbox"
                checked={selectedItemIdsBit & (1 << i)}
                class="checkbox checkbox-xs checkbox-success absolute top-2 right-2"
              />
            {/if}
              ...
              <div class="card-body w-50 p-4">
                {#if showEverything && searchString}
                  <div class="line-clamp-2 mr-3">
                    ...
                    <span class="text text-gray-400"
                      >{@html highlightSearch(item.meaning)}</span
                    >
                  </div>
                  <p class="text font-serif text-sm line-clamp-3">
                    {@html highlightSearch(item.notes)}
                  </p>
                ...
                {:else if searchString}
                  <div class="line-clamp-2 mr-3">
                    <span class="text font-semibold text-xl text-wrap mr-0.5"
                      >{@html highlightSearch(item.name)}</span
                    >
                    <span class="text meaning text-white"
                      >{@html highlightHiddenSearch(item.meaning)}</span
                    >
                  </div>
                  <p
                    class="text notes font-serif text-sm line-clamp-3 text-white"
                  >
                    {@html highlightHiddenSearch(item.notes)}
                  </p>
                {:else}
                  <div class="line-clamp-2 mr-3">
                    <span class="text font-semibold text-xl text-wrap mr-0.5"
      ...
      {#if userState.select}
        <div class="dropdown dropdown-top dropdown-end">
          <div
            tabindex="0"
            role="button"
            class="btn btn-outline btn-primary hover:text-white shadow-none"
          >
            Move to
          </div>

          <ul
            class="menu dropdown-content bg-base-300 rounded-box z-1 w-52 p-2 shadow-sm mb-2"
          >
            {#each folders as folder}
              <li>
                <a href="#" onclick={() => moveItems(folder.id)}
                  >{folder.name}</a
                >
              </li>
            {/each}
